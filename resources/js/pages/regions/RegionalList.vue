...
const navigateToRegionDetail = (regionId) => {
    router.visit(`/regions/${regionId}`);
};
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
            ...
            <div v-if="loading" class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                <Skeleton v-for="i in 6" :key="i" class="h-32 w-full rounded-lg" />
            </div>
            ...
            <div v-else-if="regions.length === 0" class="py-12">
                <Empty>
                    <EmptyHeader>
                        ...
                        <EmptyDescription>No regional data available</EmptyDescription>
                    </EmptyHeader>
                </Empty>
            </div>
            ...
            <div v-else class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                <Card v-for="region in regions" :key="region.id"
                    class="cursor-pointer transition-colors hover:bg-accent" @click="navigateToRegionDetail(region.id)">
                    ...
                    <CardContent>
                        <div class="space-y-2">
                            ...
                            <div class="flex items-center justify-between">
                                <span class="text-xs text-muted-foreground">Regional No.</span>
                                <span class="text-sm font-semibold">{{
                                    region.no
                                    }}</span>
                            </div>
                            <div class="flex items-center justify-between border-t pt-2">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-xs text-muted-foreground">Plants</span>
                                </div>
